...
// ─── 3-inch Thermal (80 mm wide) ─────────────────────────────────────────────

export async function generateThermalSalarySlipPDF(
  record: SalaryRecord,
  employee: Employee,
  ...
  const W = 80       // 3-inch thermal paper width in mm
  const H = 200      // tall enough; jsPDF clips to content on save
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: [W, H] })
  const sym = pdfSym(settings.currencySymbol ?? '₹')
  const cx  = W / 2
  let   y   = 6

  const line  = (x1: number, x2: number) => { doc.setDrawColor(0); doc.line(x1, y, x2, y); y += 3 }
  const dashes = () => { line(4, W - 4) }
  ...
  doc.setFontSize(10)
  doc.text(settings.name || 'SEP Salary Shop', cx, y, { align: 'center' })
  y += 5
  doc.setFontSize(7)
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  doc.text('SALARY SLIP', cx, y, { align: 'center' })
  y += 4.5
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(7.5)
  doc.text(`${MONTHS[record.month - 1]} ${record.year}`, cx, y, { align: 'center' })
  y += 5
  ...
  row('Base Salary',    `${sym}${record.baseSalary.toFixed(2)}`)
  row('Bonus',          `${sym}${record.bonus.toFixed(2)}`)

  dashes()

  row('Leave Deduct.',  `-${sym}${record.leaveDeduction.toFixed(2)}`)
  row('Advance',        `-${sym}${record.advanceDeduction.toFixed(2)}`)
  row('Late Deduct.',   `-${sym}${record.lateDeduction.toFixed(2)}`)
  row('Other Deduct.',  `-${sym}${record.otherDeductions.toFixed(2)}`)

  dashes()

  // Net salary — bold & large
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  doc.text('NET SALARY', 4, y)
  doc.setFontSize(11)
  doc.text(`${sym}${record.finalSalary.toFixed(2)}`, W - 4, y, { align: 'right' })
  y += 6
  ...
  doc.setFontSize(7.5)
  doc.setFont('helvetica', 'normal')
  doc.text(`Status: ${status}`, cx, y, { align: 'center' })
  y += 5
  ...
  y += 4
  doc.text('Generated by SEP Salary Shop', cx, y, { align: 'center' })

  doc.save(`thermal-slip-${employee.name.replace(/\s+/g, '-')}-${MONTHS[record.month - 1]}-${record.year}.pdf`)
}

export async function generateThermalPayrollReportPDF(
  ...
  const W   = 80
  const H   = Math.max(200, records.length * 55 + 60)
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: [W, H] })
  const sym = pdfSym(settings.currencySymbol ?? '₹')
  const cx  = W / 2
  let   y   = 6

  const dashes = () => { doc.setDrawColor(0); doc.line(4, y, W - 4, y); y += 3 }
  const row    = (label: string, value: string, bold = false) => {
    doc.setFontSize(7.5)
    doc.setFont('helvetica', bold ? 'bold' : 'normal')
    doc.setTextColor(0)
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  doc.text('PAYROLL REPORT', cx, y, { align: 'center' })
  y += 4.5
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(7.5)
  doc.text(`${MONTHS[month - 1]} ${year}  ·  ${records.length} employees`, cx, y, { align: 'center' })
  y += 5
  ...
  records.forEach(r => {
    ...
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8.5)
    doc.text(r.employeeName, 4, y)
    const statusLabel = r.paid ? '[PAID]' : '[PENDING]'
    doc.setFontSize(7)
    doc.text(statusLabel, W - 4, y, { align: 'right' })
    y += 4.5
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(7)
    doc.text(r.branch, 4, y)
    y += 4
    row('Base',    `${sym}${r.baseSalary.toFixed(2)}`)
    row('Absent',  `${r.absentDays}d  Ded: ${sym}${r.leaveDeduction.toFixed(2)}`)
    row('Advance', `-${sym}${r.advanceDeduction.toFixed(2)}`)
    row('Bonus',   `+${sym}${r.bonus.toFixed(2)}`)
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(8.5)
    doc.text('Net Salary', 4, y)
    doc.setFontSize(9)
    doc.text(`${sym}${r.finalSalary.toFixed(2)}`, W - 4, y, { align: 'right' })
    y += 5.5
  })

  dashes()

  const total = records.reduce((s, r) => s + r.finalSalary, 0)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(9)
  doc.text('TOTAL', 4, y)
  doc.text(`${sym}${total.toFixed(2)}`, W - 4, y, { align: 'right' })
  y += 6
  ...
  doc.save(`thermal-payroll-${MONTHS[month - 1]}-${year}.pdf`)
}
